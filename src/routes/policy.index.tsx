...
function PolicyIndex() {
  ...
  const visible = useMemo(
    () =>
      filter === "All"
        ? policyPosts
        : policyPosts.filter((p) => p.themes.includes(filter)),
    [filter],
  );
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      >
        <div className="max-w-6xl mx-auto px-6 relative">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold mb-4 max-w-3xl leading-tight">
            Your hub for landscape industry policy & campaigns
          </h1>
          <p className="text-blue-100 text-lg max-w-2xl">
            The latest policy updates, government consultation responses and
            campaigns from BALI's policy team — representing 1,800 businesses
            and over 950 accredited members across the UK.
          </p>
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            <a
              href="mailto:policy@bali.org.uk"
              className="bg-white text-bali-blue px-5 py-2.5 rounded-lg font-semibold hover:bg-blue-50 transition-colors"
            >
              Contact the policy team
            </a>
            <a
              href="https://bali-policy.org.uk"
              target="_blank"
              rel="noopener noreferrer"
              className="border border-white/40 px-5 py-2.5 rounded-lg font-semibold hover:bg-white/10 transition-colors"
            >
              Visit bali-policy.org.uk ↗
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 border-b border-gray-200 bg-gray-50 sticky top-[68px] z-30">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-sm uppercase tracking-widest text-gray-500 font-semibold mr-2">
              Filter by theme:
            </span>
            <FilterButton
              label="All"
              count={policyPosts.length}
              active={filter === "All"}
        ...
        <section className="py-12 bg-white">
          <div className="max-w-6xl mx-auto px-6">
            <Link
              ...
              className="group grid md:grid-cols-2 gap-8 items-stretch bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl border border-gray-100 transition-all"
            >
              {featured.image && (
                <img
              ...
              <div className="p-8 flex flex-col justify-center">
                ...
                <span className="text-xs uppercase tracking-widest text-bali-purple font-semibold">
                  Featured · {featured.date} · {featured.readMinutes} min read
                </span>
...
function ThemePill({ theme, small = false }: { theme: PolicyTheme; small?: boolean }) {
  return (
    <span
      className={`inline-block rounded-full font-semibold ${THEME_STYLES[theme]} ${
